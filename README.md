...
**WhiteNoiseRoad** — это игра с бесконечной процедурной генерацией дороги в режиме реального времени. Проект демонстрирует современные техники генерации контента, физики и реализма для игр с открытым миром.
...
* **Модульная генерация:** Система префабов с точками стыковки `StartPoint` и `EndPoint` обеспечивает идеальное соединение сегментов чанков.
* **Защита от пересечений:** Алгоритм проверки на безопасность позиции предотвращает наезд дороги самой на себя.
* **Система чанков:** Необходимо вручную нарисовать чанки определённых размеров и указать параметры в генераторе.
* **Оптимизация памяти:** Система удаления старых чанков на основе дистанции от игрока.

## 📂 Структура репозитория

* **`Assets/Scripts/`** — Все скрипты игры
* **`Assets/Prefabs/`** — Префабы дороги, окруженияб, чанков, машины и т.д.
...
## **Актуальная RoadMap находиться в Issues** 🚀 Roadmap (Планы развития) 
